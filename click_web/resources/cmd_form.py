...
def _get_commands_by_path(
    command_path: str,
) -> List[Tuple[click.Context, click.Command]]:
    """
    ...
    if command.name != command_name:
        raise CommandNotFound(
            "Failed to find root command {}. There is a root command named:{}".format(
    ...
    with click.Context(command, info_name=command, parent=None) as ctx:
        ...
        for command_name in command_path_items:
            ...
            if command:
                ...
                raise CommandNotFound(
                    'Failed to find command for path "{}". Command "{}" not found. Must be one of {}'.format(
