...
def test_exec_with_arg_and_default_opt(form_data, expected_msg, app, client):
    resp = client.post("/cli/command-with-option-and-argument", data=form_data)
    assert resp.status_code == 200
    print(resp.data)
...
def test_exec_with_default_on_flag_option(form_data, expected_msg, app, client):
    resp = client.post("/cli/command-with-default-on-flag-option", data=form_data)
    assert resp.status_code == 200
    print(resp.data)
    assert expected_msg in resp.data
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            MultiDict(
                [
                    ("1.0.option.text.2.text.--an-option", "a"),
...
def test_exec_with_nargs_opts(form_data, expected_msg, app, client):
    resp = client.post("/cli/command-with-nargs-option", data=form_data)
    assert resp.status_code == 200
    print(resp.data)
    assert expected_msg in resp.data
...
def test_exec_with_variadic_args(form_data, expected_msg, app, client):
    resp = client.post("/cli/command-with-variadic-args", data=form_data)
    assert resp.status_code == 200
    print(resp.data)
    assert expected_msg in resp.data
